...
class CNN():
    ...
    def __init__(self, input_dim=(1, 28, 28),hidden_size=256, output_size=10):
        ...
        pre_node_nums = np.array([1*3*3, 16*3*3, 32*3*3,32*3*3,64*3*3,64*3*3,128*3*3,hidden_size,128])
        ...
        conv_param4 = {'filter_num':64, 'filter_size':3, 'pad':1, 'stride':1}
        conv_param5 = {'filter_num':64, 'filter_size':3, 'pad':1, 'stride':1}
        conv_param6 = {'filter_num':128, 'filter_size':3, 'pad':1, 'stride':1}
        ...
        self.params['W7'] = weight_init_scales[6] * np.random.randn(128*3*3, hidden_size)
        self.params['b7'] = np.zeros(hidden_size)
        
        self.params['W8'] = weight_init_scales[7] * np.random.randn(hidden_size,64)
        self.params['b8'] = np.zeros(64)
        
        self.params['W9'] = weight_init_scales[8] * np.random.randn(64, output_size)
        ...
        self.layers.append(layer.Affine(self.params['W7'],self.params['b7'])) #15
        self.layers.append(layer.Relu())
        self.layers.append(layer.Affine(self.params['W8'],self.params['b8'])) #17
        self.layers.append(layer.Relu())
        self.layers.append(layer.Affine(self.params['W9'],self.params['b9'])) #19
    ...
    def gradient(self,t):
        ...
        grads = {}
        for i, layer_idx in enumerate((0, 2, 5, 7, 10, 12,15,17,19)):
